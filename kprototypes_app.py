...
st.title("基于 KPrototypes 的淘宝用户消费聚类分析")
st.markdown("技术路线：混合数据聚类（数值 + 类别）| log变换 + 标准化 + Cao初始化")
...
with st.sidebar:
    ...
    k_max = st.slider("最大簇数", 3, 15, 14)

st.header("1. 数据预处理")
st.markdown("""
- **数值特征**：消费金额（log变换 + 标准化）
- **类别特征**：商品类别、用户性别、age（保持类别型）
- **控制变量**：与 KMeans 保持一致的 4 个原始字段
""")

features = ["商品类别", "消费金额", "用户性别", "age"]
data = df.loc[:, features].copy()

data["消费金额"] = np.log1p(data["消费金额"])

scaler = StandardScaler()
data["消费金额"] = scaler.fit_transform(data[["消费金额"]])

for col in ["商品类别", "用户性别", "age"]:
    data[col] = data[col].astype("category")

cat_ind = [data.columns.get_loc(col) for col in ["商品类别", "用户性别", "age"]]

with st.expander("查看预处理后数据"):
    st.dataframe(data.head(10), use_container_width=True)

st.header("2. 确定最佳簇数")
st.markdown("使用肘部法则（cost）、轮廓系数、CH指数综合判断")
...
for k in k_range:
    kp = KPrototypes(n_clusters=k, random_state=42, n_init=5, init="Cao")
    labels = kp.fit_predict(X, categorical=cat_ind)
    costs.append(kp.cost_)
    
    num_col_idx = [i for i in range(X.shape[1]) if i not in cat_ind]
    X_num = X[:, num_col_idx].astype(float)
    silhouettes.append(silhouette_score(X_num, labels))
    ch_scores.append(calinski_harabasz_score(X_num, labels))
...
kp_final = KPrototypes(n_clusters=selected_k, random_state=42, n_init=10, init="Cao")
df["cluster"] = kp_final.fit_predict(X, categorical=cat_ind)

cluster_centers = pd.DataFrame(kp_final.cluster_centroids_, columns=features)
cluster_centers["消费金额"] = scaler.inverse_transform(np.expm1(cluster_centers["消费金额"].values.reshape(-1, 1))).flatten()
