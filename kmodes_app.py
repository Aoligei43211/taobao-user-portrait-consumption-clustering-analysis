...
st.set_page_config(page_title="KModes 用户画像分析", page_icon="", layout="wide")

st.title("基于 KModes 的淘宝用户消费聚类分析")
st.markdown("技术路线：全部类别特征（数值分箱转化）| 纯类别数据聚类")

@st.cache_data
def load_data():
    df = pd.read_csv("data/淘宝.csv")
    df["age"] = pd.cut(df["用户年龄"], bins=[17, 35, 50, 65], labels=["青年", "中年", "老年"])
    perc = [0, 0.50, 0.75, 0.92, 1]
    labels = ["D", "C", "B", "A"]
    df["消费分级"] = pd.qcut(df["消费金额"], q=perc, labels=labels)
    return df
...
data = df[["消费分级", "商品类别", "用户性别", "age"]].copy()
...
st.header("2. 确定最佳簇数")
st.markdown("使用肘部法则（cost）、轮廓系数（基于匹配距离）、CH指数（基于匹配距离）综合判断")
...
for k in k_range:
    km = KModes(n_clusters=k, random_state=42, n_init=10, init="Cao")
    labels = km.fit_predict(X)
    costs.append(km.cost_)
    
    X_numeric = np.zeros((X.shape[0], X.shape[1]))
    for i in range(X.shape[1]):
        unique_vals = np.unique(X[:, i])
        val_to_num = {v: idx for idx, v in enumerate(unique_vals)}
        X_numeric[:, i] = np.array([val_to_num[v] for v in X[:, i]])
    
    silhouettes.append(silhouette_score(X_numeric, labels, metric="euclidean"))
    ch_scores.append(calinski_harabasz_score(X_numeric, labels))

col1, col2, col3 = st.columns(3)

with col1:
    fig_elbow = go.Figure()
    fig_elbow.add_trace(go.Scatter(x=list(k_range), y=costs, mode="lines+markers", line=dict(color="#636EFA")))
    fig_elbow.update_layout(title="肘部法则 (Cost)", xaxis_title="簇数 K", yaxis_title="Cost", height=350)
    st.plotly_chart(fig_elbow, use_container_width=True)

with col2:
    fig_sil = go.Figure()
    fig_sil.add_trace(go.Scatter(x=list(k_range), y=silhouettes, mode="lines+markers", line=dict(color="#EF553B")))
    fig_sil.update_layout(title="轮廓系数", xaxis_title="簇数 K", yaxis_title="Silhouette Score", height=350)
    st.plotly_chart(fig_sil, use_container_width=True)

with col3:
    fig_ch = go.Figure()
    fig_ch.add_trace(go.Scatter(x=list(k_range), y=ch_scores, mode="lines+markers", line=dict(color="#00CC96")))
...
with st.sidebar:
    st.divider()
    st.header("选择聚类数")
    k_options = [best_k_sil, best_k_ch] + [k for k in k_range if k not in [best_k_sil, best_k_ch]]
    selected_k = st.radio("选择 K 值", k_options, index=0)

km_final = KModes(n_clusters=selected_k, random_state=42, n_init=10, init="Cao")
df["cluster"] = km_final.fit_predict(X)

cluster_centers = pd.DataFrame(km_final.cluster_centroids_, columns=["消费分级", "商品类别", "用户性别", "age"])

cluster_stats = df.groupby("cluster").agg({
    "用户ID": "count",
    "消费金额": ["mean", "median", "min", "max"],
    "消费分级": lambda x: x.mode().iloc[0],
    "商品类别": lambda x: x.mode().iloc[0],
    "用户性别": lambda x: x.mode().iloc[0],
    "age": lambda x: x.mode().iloc[0]
}).reset_index()
cluster_stats.columns = ["簇", "人数", "平均消费", "中位消费", "最低消费", "最高消费", "主导分级", "主导类别", "主导性别", "主导年龄段"]
cluster_stats["人数占比"] = (cluster_stats["人数"] / len(df) * 100).round(1)

st.subheader("3.1 各簇画像摘要")
st.dataframe(cluster_stats, use_container_width=True)

st.subheader("3.2 聚类中心（众数）")
st.dataframe(cluster_centers, use_container_width=True)

st.subheader("3.3 簇人数分布")
fig_pie = px.pie(cluster_stats, values="人数", names="簇", title=f"K={selected_k} 簇人数占比", hole=0.3)
st.plotly_chart(fig_pie, use_container_width=True)

st.subheader("3.4 消费金额分布（按簇）")
...
st.subheader("3.7 消费分级分布（按簇）")
grade_cluster = pd.crosstab(df["cluster"], df["消费分级"], normalize="index") * 100
fig_grade = go.Figure()
for col_name in grade_cluster.columns:
    fig_grade.add_trace(go.Bar(x=grade_cluster.index, y=grade_cluster[col_name], name=col_name))
fig_grade.update_layout(barmode="group", title="各簇消费分级占比", xaxis_title="簇编号", yaxis_title="占比 (%)", height=400)
